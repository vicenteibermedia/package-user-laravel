          ...
          <table class="table m-b-0">
            ...
            @foreach($users as $user) <tr>
               ...
               <td style="text-align:center;" class="with-btn ">
                 ...
                 <a @if($user->id) class="btn btn-sm btn-danger btn_borrar_usuario" @else class="btn btn-sm btn-danger btn_borrar_usuario disabled"  @endif data-idusuario={{$user->id}}>Borrar</a>
                 @if($user->activo == 1)
                   <a @if($user->id) class="btn btn-sm btn-warning btn_activo" @else class="btn btn-sm btn-warning btn_activo disabled" @endif data-idusuario={{$user->id}}>Desactivar</a>
                 @else
                   <a @if($user->id) class="btn btn-sm btn-success btn_activo" @else class="btn btn-sm btn-success btn_activo disabled" @endif data-idusuario={{$user->id}}>Activar</a>
                 @endif
